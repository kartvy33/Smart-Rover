...
def signup():
    ...
    try:
        ...
        return jsonify({"ok":False,"error":f"Account created, but email delivery failed: {exc}"}), 500
    return jsonify({"ok":True,"message":"Account created. Verify your email before logging in.","role":user["role"]})
...
def command():
    ...
    if patrol.running and command != "STOP":
...
def patrol_start():
    patrol.start()
    return jsonify({"ok":True,"patrol":patrol.status()})
...
def plate_upload():
    if "image" not in request.files:
        return jsonify({"ok":False,"error":"Upload an image field."}), 400
    try:
        result = recognize_bytes(request.files["image"].read(), source="upload")
